...
profiles = xr.open_dataarray(os.path.join("outputs/00_proc_bcm", args.experiment, "profiles.nc"))
...
turns = np.arange(10, len(profiles), turn_stride)
...
for turn, values in zip(turns, profiles_sub):
    frac = turn / turns[-1]
    offset = frac * np.max(profiles_sub) * 4.0
