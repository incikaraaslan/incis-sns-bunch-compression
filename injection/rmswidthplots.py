...
for idx, z_data in enumerate(stored_z_vals):
    ...
    # OPT EXP-TH
    if turn != 0:
        rms_width_exp.append(direct_rms(coords, exp_values))
        rms_width_sim.append(direct_rms(coords_sim_shifted, hist))


fig, ax = plt.subplots(figsize=(3.0, 5.0))
turns = np.arange(50, args.turns+1, args.turn_step)
# === PLOT ===
ax.plot(turns, np.asarray(rms_width_sim), color="red", linestyle='dashed', alpha=1.0, label="Simulation (aligned)")
ax.plot(turns, np.asarray(rms_width_exp), color="black", alpha=1.0, label="Experiment")
